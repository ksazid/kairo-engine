...
test("extracts grounded organization, product, audience, address and links", () => {
  ...
  assert.ok(result.locations.value.some((x) => x.includes("Valletta") && x.includes("Malta")));
...
test("removes scripts and resolves relative links", () => {
  const page = parsePage("https://example.test/path", 200, "<html><body><script>secret()</script><h1>Heading</h1><p>A paragraph long enough to remain in normalized evidence output.</p><a href='/about'>About us</a></body></html>");
